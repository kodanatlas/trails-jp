 ...
 *
 * 閾値0.15は実データ8ペアを本実装で実測した値に基づく（2026-08-24）。真の漏れが
 * 0.333 / 0.214 / 0.167、誤検出が 0.167 / 0 / 0 / 0 / 0。真の「中高選手権 団体」と
 * 誤検出の「千葉大OLC 技術局練習会 × 入間市OLC夏合宿」がともに 0.167 で**同値**のため、
 * 類似度では両者を分離できない。閾値を上げると真の漏れを落とすので、真をすべて拾う側に
 * 倒し、残る誤検出は MANUAL_LC_NO_MATCH で個別にミュートする方針とする。
 * 閾値に上振れの余地は無い＝この誤検出1件はメールに混じる前提。
 */
