...
describe("nameAffinity", () => {
  const fixtures: Array<{
    ...
    {
      // 既知の誤検出。normalize が「千葉大」→「千葉大学」を展開する結果 0.167 となり、
      // 真の「中高選手権 団体」(0.167) と同値＝類似度では分離不能。閾値を上げると真を落とす
      // ため likely のまま許容し、MANUAL_LC_NO_MATCH でのミュートを前提とする。
      // 値ごと固定して、normalize や係数の変更でこの均衡が動いたら気づけるようにする。
      label: "千葉大練習会と夏合宿は誤検出だが閾値を超える（真と同値で分離不能）",
      joe: "千葉大OLC 技術局練習会",
      lc: "入間市OLC夏合宿 Day2",
      expected: 0.167,
      tier: "likely",
