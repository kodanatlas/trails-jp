...
// 変更前はどちらも core 2文字で「全部未突合」だったので、未突合を減らす代わりに取り違えの
// 目を作った形。2019〜2026 の全件検証では消失・付け替えとも 0 件。「県民」は同種の効果が
// 薄い（県名は3文字以上が多い）ため据え置く。
...
const EVENT_ALIASES: [RegExp, string][] = [
  ...
  [/スプセレ/g, "スプリントセレクション"],
  [/インカレ/g, "日本学生選手権"],
  // JOY の略称と LapCenter の正式名を橋渡しする。
  // 【既知の限界】本大会名は STOP_WORDS 除去後の core が「中学校高等学校」と短く汎用的なため、
  // これを含む名前は何でも包含判定で当たる。2026-08-24 実測で「中高選手権対策&新人練習会」
  // 「中高選手権中・西日本セレ」「中高選手権 全国選考会」はいずれも接尾辞なしの本大会名と
  // MATCH する。alias を狭めると本命の「団体オープン競技」（LC 側が接尾辞なし）も落ちるため、
  // 分離できない。**同日開催でなければ実害は出ない**設計（突合は同日限定）で、2019〜2026 の
  // 全件検証では衝突ゼロ。将来ぶつかったら本大会側を MANUAL_LC_OVERRIDES で明示指定する。
  [/中高選手権/g, "全国中学校高等学校オリエンテーリング選手権"],
  // JOY の会場名と LapCenter の開催市名を橋渡しする（月次の同一体験会）。
  // 単に「入間市」へ潰すと core が市名だけになり識別力を失う（例:「彩の森入間公園OL大会」の
  // core が「入間市」になり、無関係な「入間市OLC夏合宿」の core「入間市夏合宿」に包含されて
  // 誤突合しうる）。会場名の識別部分を残した「入間市彩の森」にすることで、LC 側の core
  // 「入間市」を包含する関係は保ったまま、同市内の別大会との過剰一致を防ぐ。
  [/彩の森入間公園/g, "入間市彩の森"],
];
...
export const MANUAL_LC_OVERRIDES: Record<number, number> = {
  ...
  // 彩の森入間公園の体験会: JOY が会場名「彩の森入間公園」、LapCenter が市名「入間市」を使うため
  // 名前だけでは結び付かず、2024-04 以降の全30回が未突合だった。
  // 2026-08-24 に EVENT_ALIASES へ会場名→「入間市彩の森」の橋渡しを入れて自動突合できるように
  // したので、**今後この月次シリーズを都度ここへ追記する必要はない**（下記2件は alias 導入前の
  // 救済分。alias でも当たるが、履歴として残す）。
  2518: 9973, // 彩の森入間公園OL体験会＆併設ロゲ (2026-08-22) — LC は「第56回(26年度8月度)入間市オリエンテーリング体験会」。主催はいずれも入間市オリエンテーリングクラブ。
  2517: 9913, // 彩の森入間公園OL体験会＆併設ロゲ (2026-07-18) — LC は「第55回(26年度7月度) 入間市オリエンテーリング体験会」。上の8月分と同じ月次シリーズ。
  // 以下は 2026-08-24 に導入した突合漏れ検知（match-gaps）が最初に拾った実際の漏れ。
  // いずれも JOY の正式名・主催と LapCenter のタイトルを突き合わせて同一大会と確認済み。
  2613: 9921, // ES関東C日本代表選手壮行会2 (2026-07-20) — JOY 正式名「ES関東C日本代表選手壮行会2兼行田チャレンジ」、LC「ES関東CWUCO＆AsJWOC日本代表選手壮行会（第２弾）」。主催はいずれも ES関東C。JOY の末尾「2」と LC の「第２弾」が対応。結果は file=1 が完全版(EX/S/行田チャレンジ)。
  2552: 9875, // 金沢市民オリエンテーリング大会 (2026-06-28) — JOY 正式名「第69回金沢市民スポーツ大会（オリエンテーリング競技）」、LC「（令和８年度）第69回金沢市民スポーツ大会オリエンテーリング競技」で回数まで一致。STOP_WORDS の「市民」が削られて core が痩せるのが不成立の原因。
...
export const MANUAL_LC_NO_MATCH: Record<number, string> = {
  ...
  2434: "第49回霧ヶ峰オリエンテーリング大会 (2026-07-05・諏訪市OL協会) — mulka2 に結果掲載なし。同日の未使用LCは「OLT7月練習会」と「石川県民スポーツ大会」で別大会",
  2555: "千葉大OLC 技術局練習会 (2026-07-26) — JOY 側で削除済み(/event/view/2555 が404)。同日の未使用LCは「スプセレ対策練」「入間市OLC夏合宿 Day2」で別団体",
};
