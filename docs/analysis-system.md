...
**Stage 2c（2026-07-07 追加）**:
- **コホート帯基準（記述比較・検定なし）**: 掲載選手を中央値巡航速度の分位で帯分け（forest 5帯 / sprint 3帯・実測ミス率は単調 forest 13.8%→20.9% / sprint 9.1%→14.2%・帯セル表示は n≥2,000）。各セルの帯平均を tooltip 参考値として、また「観測差が最大のセル 約+Npt」を5pt刻み・自分のセル n≥15・差5pt以上のときのみ表示。**検定フラグとは証拠水準が異なる**（9セルの max を取る選択バイアスあり・「観測差」「検定なし」と明示）。巡航速度は出走クラス上位3基準の相対値のため帯は絶対走力ではなく、帯境界の誤配属（中央値の標本誤差≈帯幅の7割）も残る。循環性（speed がミスで汚染される懸念）は実測で否定（レース内 speed×ミス率相関 ≈ 0）。
- **フォーク構造検出は評価の上、棄却**: lap/Ave3 比の下側尾（ratio≤0.75 占有率）による構造検出を実データで検証したが、**陽性対照（フォークが実在するリレー132クラス）で信号ゼロ**＝Ave3（上位3平均）が短フォーク側に錨を下ろすため原理的に鳴らない。名前ベース除外の正規表現を fork 形式（farsta/motala/バタフライ等）へ拡張して置換（未捕捉の既知フォーククラスは800大会中1件だった）。
- **同姓同名の除外**: 「物理的に1人ではありえない ＋ クラブ表記が相違する」名前を集計から除外。条件は (i) 同一クラスにクラブ相違の rank 付き2行、(ii) 同日の別レースで走行時間帯が300秒以上重複しクラブ相違（開始・終了±60秒一致は同一走の重複掲載とみなし非該当）。クラブ条件なしの物理矛盾のみでは**732名を誤検出**（ほぼ全てが練習会の同クラス再走＝同一クラブ。クラブ相違の同クラス重複は全DBで2組のみ）した実測を受けた設計。実データでの検出は1名。**クラブ欠落データの同姓同名は検出できず残りうる**（ingest の tracked 判定は名前＋クラブ照合だが、どちらかのクラブが空なら名前のみで一致する）。cross-race.json にも同一の除外を適用。

**Stage 2d（2026-07-08 追加・期間比較）**:
- **直近12ヶ月 vs それ以前の未調整の観測ミス率**（記述のみ・検定なし）。採用レース（パック除染・レースゲート通過後のプール）を build 時点の12ヶ月前で二分し、**両期間とも 3レース以上かつ 30レッグ以上**のときだけ全体ミス率を出す（実データ掲載率 forest 54% / sprint 43%）。表示は5%刻み・**矢印や色で改善判定を示さない**・レース数を併記。
- **セル別（3×3）の期間差分は出さない**（Codex 辛口レビュー反映）: n≥15 でも ±25-30pt 揺れ、レッグはレース内相関するため実質1-2レース由来のコース差になりうる。9セルの選択バイアスもあり、permutation+BH の赤フラグとは証拠水準が違うため、薄い n で「どのセルが直った」と誤読させない。
- **交絡の明示**: 1選手の期間比較はクラス・季節・天候・コース難度を分離していないため上達/悪化の判定には使えない（方向は miss率 Theil–Sen トレンド線、この期間表示は「最近の観測値」の補助）。
