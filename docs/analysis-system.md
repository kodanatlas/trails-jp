...
| `POST /api/likes` | いいね送信（session_id + IP hash で重複防止、409 で既にいいね済み） |
| `GET /api/likes?athletes=A,B` | 指定選手のいいね数取得 |
| `GET /api/likes/top?limit=10` | いいね数上位ランキング取得 |
...
| 応援 | `SupportTab` | トレンド選手一覧・いいね・(将来) PayPay 寄付 |
...
6. **RecentEvents**: 月別参加頻度ヒートマップ + 直近 10 大会リスト（成績レベル 5 段階色分け: 好成績=水色, やや良い=緑, 平均的=黄, やや低い=橙, 低い=赤）
...
選手の調子（recentForm）に基づいてトレンド選手を一覧化し、閲覧者がいいねや PayPay 寄付で応援できる機能。

### 10.2 現在の実装（Phase 1-2）

| 機能 | 状態 | 説明 |
|---|---|---|
| トレンド一覧 | 実装済み | 調子上昇中20名 / 下降中20名（bestRank ≤ 500 かつ recentForm ≠ 0） |
| 種目バッジ | 実装済み | Forester(緑F), Sprinter(青S), Allrounder(紫F+S) |
| いいねボタン | 実装済み | 匿名OK、localStorage session_id + IP hash(SHA-256, 日次) で重複防止 |
| いいねランキング | 実装済み | いいね数上位10名を表示 |

### 10.3 今後の構想（Phase 3-4）

#### Phase 3: 選手申請 + 管理画面

選手本人が PayPay 送金リンクを登録し、管理者の手動確認を経て公開する仕組み。

- **選手申請ページ** (`/support/claim`): Email OTP 認証 → 選手名検索 → PayPay リンク入力 → 本人確認スクショアップロード → 応援メッセージ入力
- **管理画面** (`/support/admin`): スクショの氏名と選手名を目視比較して承認/却下
- **Supabase テーブル**: `athlete_claims`（申請データ + スクショパス + ステータス）
- **Supabase Storage**: `claim-screenshots` バケット（非公開、管理者のみアクセス）

**本人確認フロー:**
1. 選手が PayPay 送金リンクと本人確認画面スクリーンショットを提出
2. 管理者がスクショの氏名 ≒ 選手名を目視確認（PayPay アプリでの二重確認も可能）
3. 承認後、PayPay リンクと応援メッセージが公開

#### Phase 4: PayPay 寄付 + メッセージ

- **PayPayButton**: 承認済み選手のカードに「応援する」ボタン表示、`window.open(paypayLink)` で外部リンク
- **応援メッセージ**: 承認済み選手のみ一言メッセージ（140字以内）を投稿可能
- **免責表示**: 「寄付は PayPay アプリ上で直接行われます。当サイトは金銭の授受に関与しません」
- **`/api/athletes/approved`**: 承認済み選手の公開情報（PayPay リンク + メッセージ）を返却

**設計方針:**
- プラットフォームはお金を扱わない（リンク型）
- PayPay API 連携不要（加盟店登録なし）
- 管理者手動確認がボトルネックだが、コミュニティ規模（数百名）では十分対応可能
