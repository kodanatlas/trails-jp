...
```
/api/lapcenter-runners (Supabase, 増分データ)
  → 該当選手がいない場合 → /data/lapcenter-runners.json (静的, 全件データ)
```
**注意:** Supabase Storage のデータは Cron 増分のみ（少数）。静的ファイルが手動フルスクレイプの本体データ。API成功でも該当選手不在時は静的ファイルにフォールバックする。
