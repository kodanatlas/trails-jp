...
## 11. ローンチ後の追加対応ログ（2026-06-17〜18・本番反映済み）

本番デプロイは Vercel git連携（main push で自動ビルド）。主なコミット:
- `868abb8` 直近の大会ハイライト初版（撤去:今月の急上昇／上=ポイント上昇度・下=合成上昇度）
- `5b150e7` **巡航速度の向き修正（小さいほど速い）**＋大会名表示・複数大会は選手ごとに良い方選択
- `88c6b22` 2リストのアコーディオン化（先頭5＋最大20）＋対戦成績(H2H)候補を「成績が近いライバル」に修正
- `0b7a58e` 展開/閉じるボタン化・合成カードのスマホ崩れ修正・Heroに上部ハイライト導線・注記整理
- `b92c048` トップに「更新情報」セクション（データ更新の自動フィード）
- `4f10887` Codexレビュー反映（合成baseline粒度・H2H候補active限定・更新情報の未来日除外・文言）
- `7f0fee3` **種目(forest/sprint)誤判定の自動補正**（build-analysis-index で JOY種目を正に lc_performances.race_type をバックフィル／補助RPC `lc_distinct_events`／80件暴走ガード）
  - 本番ビルドで実行確認済み(2026-06-17 23:14): 6/13東大OLK前日大会 forest→**sprint**（n=286）／6/14本大会は forest 維持／distinct sprint大会=68件(全439中・暴走ガード未抵触)／合成リストの前日大会行が**スプリント**表示に。

### 重要な学び（実データで確定）
- 巡航速度は**小さいほど速い（良い）**（優勝者≈最小値で実証）。[[reference_trails_lapcenter_metric_direction]] にも記録。
- `lc_performances.race_type` はイベント名キーワード判定で、キーワード無しスプリント（例:東大OLK前日大会=park sprint）が forest に誤分類。他機能はJOYランキング種目（authoritative）で判定していた→合成も同基準に揃えた。

### Codex(gpt-5.5)レビューで未対応（要検討・優先度中〜低）
- **H2H 対戦履歴の重複排除キー** `date:eventName` が type/className/discipline を落とす→同日のForest/Sprint等が片方消え得る（既存実装・本件外）。
- **H2H 同姓同名ガード**が候補チップのみ。検索選択経由は未通過＋`hasMergedNamesakes`は同(type,className)重複しか検出しない（既存実装）。
- **H2H 対戦履歴/再戦予定テーブルのモバイル幅**が固定的で小幅端末で欠ける可能性（既存実装。`overflow-x-hidden`配下）。
- **合成リスト下部の更新時刻**は ISR レンダー時刻で、実データ/cron時刻ではない（cron停止時も「毎日更新」に見える）。
- **H2H候補の近さ指標**は共通クラスの順位差(最小)＋isActive限定までは対応。フィールドサイズ差/累積年齢クラスの重み付けは未対応（実例では妥当な抽出を確認済み）。
- **自動テスト不足**: H2H候補ロジック・`weekend_standouts` SQL はデータ依存が強いがユニットテスト無し（実データスモークで代替中）。

### 既知の運用前提
- race_type バックフィルは**ビルド毎に自己修復**（JOYランキング対象外の小規模大会のみ従来キーワードにフォールバック）。新規イベントの即時種目判定（sync-lapcenter側）は未実装＝次ビルドまで最大1週間ラグ。
- carpool 関連の未コミット変更3ファイル（`src/lib/carpool/geocode.ts` ほか）は本作業と無関係のため一貫して手つかず。
